...
提供训练(seq_train)和评估(seq_eval)的核心循环,以及辅助函数
...
def _to_device(batch):
    """把 collate 出的 (vid, vid_lgt, label, label_lgt) 四元组移入当前设备。

    CUDAPrefetcher 命中时会直接给出已在设备上的张量,DeviceManager.to 对
    同设备张量是廉价的 no-op,这里无需再判断。
    """
    vid, vid_lgt, label, label_lgt = (DeviceManager.to(batch[index]) for index in range(4))
    return {
        Keys.VID: vid,
        Keys.VID_LGT: vid_lgt,
        Keys.LABEL: label,
        Keys.LABEL_LGT: label_lgt,
    }
...
def _save_sample_statistics(stats, work_dir, mode):
    ...
    Args:
        stats: SampleStatistics 对象。
        work_dir: 实验工作目录。
        mode: 评估模式("dev"/"test")。

    Returns:
        pathlib.Path: 统计文件路径;保存失败时返回 None。
    """
...
def get_feeder_arg(cfg, key, default=None):
    ...
    可用的 key 受 ConfigManager.KNOWN_NESTED_KEYS["feeder_args"] 约束。
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir):
    ...
    try:
        for batch_idx, data in enumerate(tqdm(iterator)):
            batch_info = data[-1]
            batch_ids = _sample_ids(batch_info)
