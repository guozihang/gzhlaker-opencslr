...
class Decoder(object):
    """CTC decoder, and the `decoder` slot of the four-container contract.

    Use ``decode()`` directly to decode a batch of logits, or call the instance
    on a data dict to run as the pipeline's decoder stage: it reads
    ``SEQUENCE_LOGITS`` and ``FEAT_LEN``, and writes ``RECOGNIZED_SENTS`` back.

    Args:
        args: Config dict containing "num_classes" and optionally "decode_mode"
            ("max" for greedy, anything else for beam search; default "beam").
        gloss_dict: Dictionary mapping gloss indices to (gloss_string, ...) tuples.
        blank_id: Index of the CTC blank token (default 0).
    """

    def __init__(self, args, gloss_dict, blank_id=0):
        num_classes = args["num_classes"]
        self.i2g_dict = dict((v[0], k) for k, v in gloss_dict.items())
        self.search_mode = args.get("decode_mode", "beam")
    ...
    def __call__(self, data):
        ...
        pred = self.decode(data[Keys.SEQUENCE_LOGITS], data[Keys.FEAT_LEN],
